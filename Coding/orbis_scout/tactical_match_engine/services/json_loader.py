...
def get_league_info(country: str,
    ...
    try:
        ...
        return {"intensity": 0.5, "cvs": 50.0, "global_rank": None, "name": "", "is_estimated": True}
...
def load_sofascore_player(file_path: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "current_league_intensity_estimated": league_info.get("is_estimated", False),
...
def load_sofascore_player_from_doc(doc: Dict[str, Any]) -> Dict[str, Any]:
    """
    Convert a MongoDB player document into the same dict shape returned by
    load_sofascore_player(), without reading any file from disk.

    The document must come from the ``players`` collection (same structure as
    the scraped JSON files imported by mongo_import.py).
    """
    profile  = doc.get("profile", {}).get("player", {})
    raw_stats: Dict = doc.get("statistics", {}).get("statistics", {})
    ...
    # MongoDB tags (_country / _competition) take precedence over the original
    # competition_country / competition_name fields inside the document.
    country     = doc.get("_country") or doc.get("competition_country", "")
    competition = doc.get("_competition") or doc.get("competition_name", "")
    ...
    return {
        "id":                                str(doc.get("player_id", "")),
        "name":                              doc.get("player_name") or profile.get("name", "Unknown"),
        "age":                               age,
        "position":                          primary_position,
        "positions":                         positions_detailed,
        "preferred_foot":                    profile.get("preferredFoot", "Right"),
